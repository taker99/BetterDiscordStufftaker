...
module.exports = (() => {
	const config = {
		info: {
			...
			description:
				"Shows an indicator in the guild/channel list when someone is typing there",
			version: "0.5.4",
			github:
				"https://github.com/l0c4lh057/BetterDiscordStuff/blob/master/Plugins/TypingIndicator/",
			github_raw:
		...
		changelog: [
			{
				...
				type: "fixed",
				items: [
					"Indicator visible again when using light theme (those were the worst two minutes of this year, even with my health issues rn)",
				],
			},
		],
	};
	...
	return !global.ZeresPluginLibrary
		...
		: (([Plugin, Api]) => {
				const plugin = (Plugin, Api) => {
					...
					const MutedStore = WebpackModules.getByProps(
						"isMuted",
						"isChannelMuted",
					);
					...
					if (
						!BdApi.Plugins.get("BugReportHelper") &&
						!BdApi.getData(config.info.name, "didShowIssueHelperPopup")
					) {
								...
								onConfirm: () => {
									require("request").get(
										...
										(error, response, body) => {
											if (error)
												return require("electron").shell.openExternal(
													"https://betterdiscord.net/ghdl?url=https://raw.githubusercontent.com/l0c4lh057/BetterDiscordStuff/master/Plugins/BugReportHelper/BugReportHelper.plugin.js",
												);
											else
												require("fs").writeFile(
													require("path").join(
														BdApi.Plugins.folder,
														"BugReportHelper.plugin.js",
													),
													body,
													() => {
														window.setTimeout(
					...
					`,
						...
						getPrivateChannels() {
							return ChannelStore.getPrivateChannels
								? Object.values(ChannelStore.getPrivateChannels())
						...
						patchChannelList() {
							const ChannelItem = WebpackModules.getModule(
								(m) => Object(m.default).displayName === "ChannelItem",
							);
							Patcher.after(
								ChannelItem,
								"default",
								(_, [props], returnValue) => {
									if (
										props.channel.type !==
										DiscordConstants.ChannelTypes.GUILD_TEXT
									)
										return;
									...
									if (!selfId)
										return setTimeout(() => this.patchChannelList(), 100);
									...
									const itemList = Utilities.getNestedProp(
										returnValue,
						...
						patchGuildList(promiseState) {
							const GuildComponents = WebpackModules.getModule(
								(m) => m?.default?.displayName === "GuildNode",
							);
							if (
								!GuildComponents ||
								typeof GuildComponents.default !== "function"
							)
								return console.error(
									"[TypingIndicator] Could not find Guild components",
								);
							const selfId = UserStore.getCurrentUser()?.id;
							if (!selfId)
							...
							const PatchedGuild = ({ __TI_original, ...props }) => {
								...
								try {
									...
									returnValue.props.children.props.children.push(
										Indicator({ guildId: props.guild.id }),
									);
								} catch (err) {
						...
						async patchHomeIcon(promiseState) {
							...
							Patcher.after(
								Home.component.prototype,
								"render",
								(thisObject, _, returnValue) => {
									if (!returnValue.props.children) return;
									let children =
										returnValue.props.children[0] || returnValue.props.children;
									if (!children.props) return;
									if (!children.props.children || !children.props.className)
										return;
									if (
									...
									const fluxWrapper = Flux.connectStores(
										[UserTypingStore, WindowInfo],
										() => ({
											userIds: this.getPrivateChannels()
												.filter(
													(c) =>
														this.settings.includeMuted ||
												...
												.flatMap((c) =>
													Object.keys(
														UserTypingStore.getTypingUsers(c.id),
													).filter(
														(uId) =>
															uId !== selfId &&
															(this.settings.includeBlocked ||
							...
							const Folder = WebpackModules.find(
								(m) =>
									...
									(m?.type?.render || m?.type?.__powercordOriginal_render)
										?.toString()
										?.indexOf("SERVER_FOLDER") !== -1,
							);
							if (promiseState.cancelled || !Folder) return;
							const selfId = UserStore.getCurrentUser()?.id;
							if (!selfId)
								return setTimeout(() => this.patchFolders(promiseState), 100);
							Patcher.after(
								Folder.type,
								"render",
								(_, [props], returnValue) => {
											...
											userIds: this.getGuildChannels(...props.guildIds)
												.filter(
													(c) =>
														...
														(this.settings.includeMuted ||
															!MutedStore.isChannelMuted(c.guild_id, c.id)) &&
