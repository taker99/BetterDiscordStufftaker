...
class GuildData {
	...
	getDescription() {
		return "Shows information about guilds, channels and roles by right clicking the guild's icon in the guild list.";
	}
	getVersion() {
		return "2.0.8";
	}
	...
	start() {
		...
		if (global.ZeresPluginLibrary) this.initialize();
			...
			BdApi.alert(
				"Local version of ZeresPluginLibrary required",
				"For this plugin to work you need to have the ZeresPluginLibrary plugin installed. " +
	...
	getSettingsPanel() {
		...
		new ZLibrary.Settings.SettingGroup(this.getName(), { shown: true })
			.appendTo(panel)
			.append(
				new ZLibrary.Settings.Slider(
					"Emoji Columns",
					"How many columns are used in the emoji list",
					1,
					20,
					this.settings.emojiColCount,
					(e) => {
			...
			.append(
				new ZLibrary.Settings.Textbox(
					"Date format",
					"How dates are displayed in this plugin",
			...
			.append(
				new ZLibrary.Settings.SettingGroup("Date variables").append(
					$(`<div>
			...
			.append(
				new ZLibrary.Settings.Slider(
					"Max shown user count",
					"max amount of users shown in the member list (more users -> longer loading time), default: 100",
					50,
					1000,
			...
			.append(
				new ZLibrary.Settings.Dropdown(
					"Language",
					"The language of this plugin (has no effect yet)",
	...
	initialize() {
		...
		ZLibrary.PluginUpdater.checkForUpdate(
			this.getName(),
			this.getVersion(),
			"https://raw.githubusercontent.com/l0c4lh057/BetterDiscordStuff/master/Plugins/GuildData/GuildData.plugin.js",
		);
		$(document.body).on("contextmenu.guilddata", (e) => {
			let el = e.target.parentElement;
			if (!el.href) el = e.target;
			if (!el.href) return;
			if (el.href.includes("/channels/@me/"))
				return; /* return if it's a private channel/group channel */
			if (!el.href.match(/\/channels\/[0-9]+\/[0-9]+/))
				return; /* return if it's not a guild link */
			...
			process.nextTick(() => {
				...
				let subMenu = $(
					`<div><div role="separator" class="separator-2I32lJ"></div><div role="group"><div role="menuitem" class="item-1tOPte labelContainer-1BLJti colorDefault-2K3EoJ" id="show-guild-data" tabindex="-1"><div class="label-22pbtT">Show Guild Data</div></div></div></div>`,
				)[0];
		...
		$(document.body).on("click.guilddata", (e) => {
			...
			if (
				!e.target.closest(".guilddata-copy") &&
				!e.target.classList.contains("guilddata-copy")
			)
				return;
			let toCopy = e.target.getAttribute("copyText");
			if (e.target.classList.contains("guilddata-image") && !toCopy)
				toCopy = e.target.src;
		...
		$(document.body).on("click.guilddata", (e) => {
			...
			if (
				!e.target.classList.contains("container-2Rl01u") &&
				!e.target.parentNode.classList.contains("container-2Rl01u")
			)
			...
			window.setTimeout(() => {
				...
				menu.parentNode.css(
					"height",
					`${menu.parentNode.clientHeight + separator.offsetHeight + el.offsetHeight}px`,
				);
			}, 350);
		});

		if (!this.settings.lastUsedVersion) {
	...
	showPopup(gId) {
		...
		let friends = members.filter((m) =>
			relationships[m.userId] ? relationships[m.userId] == 1 : false,
		);
		let blocked = members.filter((m) =>
			relationships[m.userId] ? relationships[m.userId] == 2 : false,
		);
		let popup = $(
		...
		if (document.getElementById("guilddata-popup"))
			document.getElementById("guilddata-popup").outerHTML = "";
		if (!o) {
			ZLibrary.DiscordModules.APIModule.get(
				ZLibrary.DiscordModules.DiscordConstants.Endpoints.USER(g.ownerId),
			).then((result) => {
				...
				popup.querySelector("#guilddata-guildowner").innerText =
					`${o.tag} (${g.ownerId})`;
			});
		}

		popup
			.querySelector("#guilddata-guildowner")
			.addEventListener("click", () => {
				this.showUser(g.id, o.id);
			});
		if (g.systemChannelId)
			popup
				.querySelector("#guilddata-systemchannel")
				.addEventListener("click", () => {
					this.showChannel(g.systemChannelId);
				});
		if (g.afkChannelId)
			popup
				.querySelector("#guilddata-afkchannel")
				.addEventListener("click", () => {
					this.showChannel(g.afkChannelId);
				});
		popup
			.querySelector("#guilddata-guildemojis")
			.addEventListener("click", () => {
				this.showEmojis(g.id);
			});
		popup
			.querySelector("#guilddata-guildfriends")
			.addEventListener("click", () => {
				this.showRelationships(g.id, friends, "friends");
			});
		popup
			.querySelector("#guilddata-guildblockedusers")
			.addEventListener("click", () => {
				this.showRelationships(g.id, blocked, "blocked");
			});
		popup
			.querySelector("#guilddata-usersearchbutton")
			.addEventListener("click", () => {
				this.showUsers(
					g.id,
					popup.querySelector("#guilddata-usersearchinput").value,
				);
			});
		popup
			.querySelector("#guilddata-usersearchinput")
		...
		popup
			.querySelector("#guilddata-serverboosting")
			.addEventListener("click", () => {
				this.showUsers(g.id, "#boosting:true");
			});
		...
		if (!g.vanityURLCode)
			popup.querySelector("#guilddata-vanityurl").outerHTML = "";
		$(".guilddata-panel").css(
			"height",
			`calc(100% - ${document.querySelector(".guilddata-panel").offsetTop}px)`,
		);
		this.showUsers(g.id, "");
	...
	showUsers(gId, query = "") {
		let { GuildStore, GuildMemberStore, UserStore, UserStatusStore } =
			ZLibrary.DiscordModules;
		let g = GuildStore.getGuild(gId);
		...
		if (query.match(/#(?![0-9])/)) {
			for (let arg of query.split("#")) {
				...
				if (k == "nick")
					members = members.filter((m) =>
						(m.nick || UserStore.getUser(m.userId).username)
							.toLowerCase()
							.includes(v.toLowerCase()),
					);
				if (k == "name")
					members = members.filter((m) =>
						UserStore.getUser(m.userId)
							.username.toLowerCase()
							.includes(v.toLowerCase()),
					);
				if (k == "nicked")
					members = members.filter((m) =>
						v.toLowerCase() == "true" ? m.nick : !m.nick,
					);
				if (k == "id") members = members.filter((m) => m.userId.includes(v));
				if (k == "discriminator")
					members = members.filter((m) =>
						UserStore.getUser(m.userId).discriminator.includes(v),
					);
				if (k == "status")
					members = members.filter((m) =>
						UserStatusStore.getStatus(m.userId).includes(v.toLowerCase()),
					);
				if (k == "role")
					members = members.filter(
						(m) =>
							Object.values(g.roles).filter(
								(r) =>
				...
				if (k == "roleid")
					members = members.filter(
						(m) =>
							m.roles.filter((r) => r.includes(v)).length > 0 ||
							g.id.includes(v),
					);
				if (k == "bot")
					members = members.filter(
						(m) =>
							UserStore.getUser(m.userId).bot == (v.toLowerCase() == "true"),
					);
				if (k == "boosting")
					members = members.filter(
						(m) => !!m.premiumSince == (v.toLowerCase() == "true"),
					);
				...
				if (k == "discriminator!")
					members = members.filter(
						(m) => !UserStore.getUser(m.userId).discriminator.includes(v),
					);
				if (k == "status!")
					members = members.filter(
						(m) =>
			...
			members = members.filter(
				(m) =>
					...
					(m.nick || UserStore.getUser(m.userId).tag)
						.toLowerCase()
						.includes(query.toLowerCase()),
			);
		}
		...
		if (members.length > this.settings.maxUsersShown) {
			...
			if (this.settings.showUsernamesInMemberlist) {
				for (let m of members) {
					let el = $(
						`<div class="guilddata-usersearchresult" id="guilddata-usersearchresult u${m.userId}">${this.escapeHtml(UserStore.getUser(m.userId).tag)}</div>`,
					)[0];
				...
				for (let m of members) {
					let el = $(
						`<div class="guilddata-usersearchresult" id="guilddata-usersearchresult u${m.userId}">${this.escapeHtml(m.nick || UserStore.getUser(m.userId).name)}#${UserStore.getUser(m.userId).discriminator}</div>`,
					)[0];
					el.addEventListener("click", () => {
						this.showUser(g.id, m.userId);
					});
		...
		btn.addEventListener("click", () => {
			this.alertText(
				"Export users",
				"You are exporting all users that match the current search.<br>In which format do you want to export the users?<br><br>%name% - username<br>%nick% - nickname or username if no nick is set<br>%discriminator% - the discriminator of the user<br>%id% - the user id<br><input id='guilddata-exportusersformat' value='%name%#%discriminator% (%id%)' style='width:90%;'>",
				() => {
					let format = document.getElementById(
						"guilddata-exportusersformat",
					).value;
					let formatMember = (m) => {
						let u = UserStore.getUser(m.userId);
						return format
							.replace(/%name%/g, u.username)
							.replace(/%nick%/g, m.nick || u.username)
							.replace(/%discriminator%/g, u.discriminator)
							.replace(/%id%/, m.userId);
					};
					this.downloadFile(
						members.map((m) => formatMember(m)).join("\r\n"),
						`users of ${g.name} - ${query}.txt`,
						`Export users of  ${g.name} that match ${query}`,
					);
				},
			);
		});
		let btn2 = $(
			`<button id="guilddata-forceloadusers">Load All Users</button>`,
		)[0];
	...
	async showUser(gId, uId) {
		...
		if (!u) {
			let us = await ZLibrary.DiscordModules.APIModule.get(
				ZLibrary.DiscordModules.DiscordConstants.Endpoints.GUILD_MEMBER(
					gId,
					uId,
				),
			);
			...
			u = {
				tag: us.user.username + "#" + us.user.discriminator,
				username: us.user.username,
				discriminator: us.user.discriminator,
				id: us.user.id,
				avatarURL: `https://cdn.discordapp.com/avatars/${us.user.id}/${us.user.avatar}.png?size=128`,
				nick: us.nick,
				roles: us.roles,
			};
			m = u;
		...
		let panel =
			$(`<div class="guilddata-panel" id="guilddata-userinfo"><div class="guilddata-closebutton"></div>
			...
			<table class="guilddata-table">
				...
				<tr class="guilddata-tr">
					...
					<td class="guilddata-td">${m.roles
						.sort((r1, r2) => g.roles[r2].position - g.roles[r1].position)
						.map(
							(rId) =>
								`<div class="guilddata-memberrole r${rId}">${this.escapeHtml(g.roles[rId].name)} (${rId})</div>`,
						)
						.join(
							"",
						)}<div class="guilddata-memberrole r${g.id}">${this.escapeHtml(g.roles[g.id].name)} (${g.id})</div></td>
				</tr>
				<tr class="guilddata-tr">
					<td class="guilddata-td">Created at</td>
		...
		$(panel).css(
			"height",
			`${document.getElementById("guilddata-channelsinfo").offsetHeight}px`,
		);

		if (!m.nick) panel.querySelector("#guilddata-usernick").outerHTML = "";
		if (!m.premiumSince)
			panel.querySelector("#guilddata-boostingsince").outerHTML = "";
		if (m.hoistRoleId)
			panel
				.querySelector("#guilddata-hoistrole")
				.addEventListener("click", () => {
					this.showRole(g.id, m.hoistRoleId);
				});

		let chatBtn = $(
			`<button class="guilddata-openprivatechat">Open Chat</button>`,
		)[0];
		chatBtn.addEventListener("click", () => {
			PrivateChannelActions.ensurePrivateChannel(
		...
		for (let rId of m.roles) {
			panel
				.querySelector(`.guilddata-memberrole.r${rId}`)
				.addEventListener("click", () => {
					this.showRole(g.id, rId);
				});
		}
		panel
			.querySelector(`.guilddata-memberrole.r${g.id}`)
			.addEventListener("click", () => {
				this.showRole(g.id, g.id);
			});
		panel
			.querySelector(".guilddata-closebutton")
			.addEventListener("click", () => {
				chatBtn.outerHTML = "";
			});
	...
	showChannels(gId) {
		let { GuildStore, ChannelStore, DiscordPermissions, GuildPermissions } =
			ZLibrary.DiscordModules;
		let g = GuildStore.getGuild(gId);
		let channels = Object.values(ChannelStore.getChannels()).filter(
			(c) => c.guild_id == g.id,
		);
		let categories = channels.filter((c) => c.type == 4);
		let textchannels = channels.filter(
		...
		for (let i = textchannels.length - 1; i >= 0; i--) {
			...
			if (!c.parent_id) {
				let el = $(
					`<div class="guilddata-channel guilddata-textchannel guilddata-noparentchannel ${GuildPermissions.can(DiscordPermissions.VIEW_CHANNEL, { channelId: c.id }) ? "guilddata-viewable" : "guilddata-notviewable"}"><span class="guilddata-channelicon">${this.getChannelIcon(c.type, c.nsfw, false, c.id)}</span><span class="guilddata-channelname">${this.escapeHtml(c.name)}</span></div>`,
				)[0];
				el.querySelector(".guilddata-channelname").addEventListener(
					"click",
					() => {
						this.showChannel(c.id);
					},
				);
				list.appendChild(el);
		...
		for (let i = voicechannels.length - 1; i >= 0; i--) {
			...
			if (!c.parent_id) {
				let el = $(
					`<div class="guilddata-channel guilddata-voicechannel guilddata-noparentchannel ${GuildPermissions.can(DiscordPermissions.VIEW_CHANNEL, { channelId: c.id }) ? "guilddata-viewable" : "guilddata-notviewable"}"><span class="guilddata-channelicon">${this.getChannelIcon(c.type, c.nsfw, false, c.id)}</span><span class="guilddata-channelname">${this.escapeHtml(c.name)}</span></div>`,
				)[0];
				el.querySelector(".guilddata-channelname").addEventListener(
					"click",
					() => {
		...
		for (let i = categories.length - 1; i >= 0; i--) {
			...
			el.querySelector(".guilddata-channelicon").addEventListener(
				"click",
				() => {
					let el2 = el.querySelector(".guilddata-categorychannels");
					if (el2.style.display == "none") {
						el2.style.display = "block";
						el.querySelector(".guilddata-channelicon").innerHTML =
							this.getChannelIcon(c.type, c.nsfw, false, c.id);
					} else {
						el2.style.display = "none";
						el.querySelector(".guilddata-channelicon").innerHTML =
							this.getChannelIcon(c.type, c.nsfw, true, c.id);
					}
				},
			);
	...
	showChannel(cId) {
		...
		let canSee = GuildPermissions.can(DiscordPermissions.VIEW_CHANNEL, {
			channelId: c.id,
		});
		let wrapper = document.getElementById("guilddata-channelwrapper");
		let panel =
			$(`<div class="guilddata-panel" id="guilddata-channelinfo"><div class="guilddata-closebutton"></div>
		...
		for (let po of Object.values(c.permissionOverwrites)) {
			if (po.type == "member") {
				let el = $(
					`<div>Member: ${this.escapeHtml(UserStore.getUser(po.id).tag)} (${po.id})</div>`,
				)[0];
				el.addEventListener("click", () => {
					this.showPermissionOverwrite(c.id, po.id, po.type);
				});
				list.appendChild(el);
			} else if (po.type == "role") {
				let el = $(
					`<div>Role: ${this.escapeHtml(g.roles[po.id].name)} (${po.id})</div>`,
				)[0];
				el.addEventListener("click", () => {
					this.showPermissionOverwrite(c.id, po.id, po.type);
				});
		...
		if (canSee && (c.type == 0 || c.type == 2 || c.type == 5)) {
			...
			if (c.type == 0 || c.type == 5) {
				openBtn = $(
					`<button class="guilddata-openchannel">Open Channel</button>`,
				)[0];
				openBtn.addEventListener("click", () =>
					ChannelSelector.selectChannel(g.id, c.id),
				);
			} else {
				openBtn = $(
					`<button class="guilddata-openchannel">Connect</button>`,
				)[0];
				openBtn.addEventListener("click", () =>
					ChannelActions.selectVoiceChannel(g.id, c.id),
				);
			}
			panel
				.querySelector(".guilddata-closebutton")
				.addEventListener("click", () => (openBtn.outerHTML = ""));
	...
	showPermissionOverwrite(cId, pId, type) {
		...
		let wrapper = document.getElementById("guilddata-channelwrapper");
		let panel =
			$(`<div class="guilddata-panel" id="guilddata-channelpermissionoverwrite"><div class="guilddata-closebutton"></div>
			<div class="guilddata-title guilddata-copy"><span class="guilddata-channelname">${this.escapeHtml(c.name)}</span> (<span class="guilddata-channelid">${c.id}</span>)</div>
		...
		panel
			.querySelector(".guilddata-permissionoverwrite")
			.addEventListener("click", () => {
				if (type == "member") this.showUser(g.id, pId);
				else this.showRole(g.id, pId);
			});
	...
	showRoles(gId) {
		...
		for (let r of roles) {
			let el = $(
				`<div class="guilddata-role">${this.escapeHtml(r.name)} (${r.id})</div>`,
			)[0];
			el.addEventListener("click", () => {
				this.showRole(g.id, r.id);
			});
			list.appendChild(el);
		}
		if (document.getElementById("guilddata-exportroles"))
			document.getElementById("guilddata-exportroles").outerHTML = "";
		let btn = $(`<button id="guilddata-exportroles">Export Roles</button>`)[0];
		btn.addEventListener("click", () => {
			this.alertText(
				"Export roles",
				"You are exporting all rolles of this server.<br>In which format do you want to export the roles?<br><br>%name% - name of the role<br>%id% - the role id<br>%userscount% - the count of users with that role<br><input id='guilddata-exportrolesformat' value='%name% (%id%)' style='width:90%;'>",
				() => {
					let members = GuildMemberStore.getMembers(g.id);
					let format = document.getElementById(
						"guilddata-exportrolesformat",
					).value;
					let formatRole = (r) => {
						return format
							...
							.replace(
								/%usercount%/g,
								members.filter((m) => m.roles.includes(r.id)).length,
							);
					};
					this.downloadFile(
						roles.map((r) => formatRole(r)).join("\r\n"),
						`roles of ${g.name}.txt`,
						`Export roles of  ${g.name}`,
					);
				},
			);
		});
	...
	showRole(gId, rId) {
		...
		let panel =
			$(`<div class="guilddata-panel" id="guilddata-roleinfo"><div class="guilddata-closebutton"></div>
		...
		$(panel).css(
			"height",
			`${document.getElementById("guilddata-rolesinfo").offsetHeight}px`,
		);
		...
		panel
			.querySelector("#guilddata-rolemembers")
			.addEventListener("click", () => {
				this.showUsers(g.id, `#roleid:${r.id}`);
			});
	...
	compareRoles(gId, rId1, rId2) {
		...
		let wrapper = document.getElementById("guilddata-rolewrapper");
		let panel =
			$(`<div class="guilddata-panel guilddata-rolecomparison"><div class="guilddata-closebutton"></div>
	...
	showEmojis(gId) {
		...
		let panel =
			$(`<div class="guilddata-panel guilddata-guildemojis"><div class="guilddata-closebutton"></div>
			<div class="guilddata-title">Emojis</div>
			<div class="guilddata-emojis"></div>
		</div>`)[0];
		$(panel).css(
			"height",
			`${document.getElementById("guilddata-guildinfo").offsetHeight}px`,
		);
		...
		for (let e of emojis) {
			let el = $(
				`<div class="guilddata-emojiwrapper"><img src="${e.url}" class="guilddata-emojiimg">${colCnt < 10 ? `<div class="guilddata-emojiname">${this.escapeHtml(e.name)}</div></div>` : ""}`,
			);
		...
		if (emojis.length == 0) {
			let el = $(
				`<div class="guilddata-noelement">This guild has no emojis.</div>`,
			)[0];
		}

		let btn = $(
			`<button class="guilddata-exportemojis">Save Emojis</button>`,
		)[0];
		btn.addEventListener("click", () => this.downloadEmojis(emojis));
		panel
			.querySelector(".guilddata-closebutton")
			.addEventListener("click", () => (btn.outerHTML = ""));
	...
	showEmoji(gId, eId) {
		...
		let g = GuildStore.getGuild(gId);
		let e = EmojiUtils.getGuildEmoji(g.id).filter(
			(emoji) => emoji.id == eId,
		)[0];

		let wrapper = document.getElementById("guilddata-guildwrapper");
		let panel =
			$(`<div class="guilddata-panel guilddata-emoji"><div class="guilddata-closebutton"></div>
		...
		if (e.roles.length > 0) {
			list.appendChild(
				$(`<div class="guilddata-emojirolesheader">Whitelisted Roles</div>`)[0],
			);
		}
		for (let rId of e.roles) {
			let r = g.roles[rId];
			let el = $(
				`<div class="guilddata-emojirole">${this.escapeHtml(r.name)} (${r.id})</div>`,
			)[0];
	...
	showRelationships(gId, members, type) {
		...
		let wrapper = document.getElementById("guilddata-guildwrapper");
		let panel =
			$(`<div class="guilddata-panel" id="guilddata-guildrelationships"><div class="guilddata-closebutton"></div>
		...
		for (let m of members) {
			let u = UserStore.getUser(m.userId);
			let el = $(
				`<div class="guilddata-relationshipuser"><img src="${u.avatarURL}" class="guilddata-relationshippfp"> <span class="guilddata-relationshipname">${this.escapeHtml(u.tag)}</span></div>`,
			)[0];
	...
	getVerificationLevel(level) {
		if (level == 0) return "Unrestricted";
		else if (level == 1) return "Verified email needed";
		else if (level == 2) return "Registered for more than 5 minutes";
		else if (level == 3) return "Guild member for at least 10 minutes";
		else if (level == 4) return "Verified phone number";
	}

	getExplicitContentFilter(level) {
		if (level == 0) return "Disabled";
		else if (level == 1) return "Members without role";
		else if (level == 2) return "All messages";
	}

	getDefaultMessageNotifications(defMesNot) {
		if (defMesNot == 0) return "All messages";
		else if (defMesNot == 1) return "Mentions only";
	}
	...
	getPermissions(permNumber) {
		...
		for (let i = allPerms.length - 1; i >= 0; i--) {
			...
			if (permNumber >= pe) {
				permNumber -= pe;
				if (generalPerms.includes(pe))
					p["general"].push(
						perms[0].permissions.filter((per) => per.flag == pe)[0].title,
					);
				if (textPerms.includes(pe))
					p["text"].push(
						perms[1].permissions.filter((per) => per.flag == pe)[0].title,
					);
				if (voicePerms.includes(pe))
					p["voice"].push(
						perms[2].permissions.filter((per) => per.flag == pe)[0].title,
					);
			}
		}
		return p;
	...
	loadSettings() {
		this.settings = ZLibrary.PluginUtilities.loadSettings(
			this.getName(),
	...
	makeDraggable() {
		...
		$("#guilddata-popup").on("mousedown", (e) => {
			if (e.target.id != "guilddata-popup" && e.target.id != "guilddata-inner")
				return;
			e.preventDefault();
			posX = e.clientX;
			posY = e.clientY;
			$(document.body).on("mousemove.guilddatadrag", (e2) => {
				...
				$("#guilddata-popup").css(
					"top",
					$("#guilddata-popup").offset().top - posY + e2.clientY + "px",
				);
				$("#guilddata-popup").css(
					"left",
					$("#guilddata-popup").offset().left - posX + e2.clientX + "px",
				);
				posX = e2.clientX;
				posY = e2.clientY;
			});
		...
		return format
			...
			.replace(
				...
				(date.getTimezoneOffset() < 0 ? "-" : "+") +
					Math.floor(Math.abs(date.getTimezoneOffset() / 60)).pad() +
					":" +
					Math.floor(Math.abs(date.getTimezoneOffset() % 60)).pad(),
			)
			.replace(
	...
	alertText(e, t, callbackOk, callbackCancel) {
		let backdrop = $(
			`<div class="backdrop-1wrmKB da-backdrop" style="opacity: 0.85; background-color: rgb(0, 0, 0); z-index: 1000; transform: translateZ(0px);"></div>`,
		);
		let a =
			$(`<div class="modal-3c3bKg da-modal" style="opacity: 1; transform: scale(1) translateZ(0px);">
		...
		let modalRoot = document.querySelector(
			"#app-mount > div:not([class]):not([style])",
		);
	...
	downloadFile(content, filename, title) {
		...
		dialog.showSaveDialog(
			{ defaultPath: filename, title: title },
			function (sel) {
				if (!sel) return;
				let fs = require("fs");
			...
			for (let e of emojis) {
				...
				request(e.url).pipe(
					fs.createWriteStream(
						`${sel}\\${e.id} - ${e.name}.${e.url.split(".").reverse()[0].split("?")[0]}`,
					),
				);
			}
		});
	...
	getSnowflakeCreationDate(id) {
		...
		return new Date(
			parseInt(toBinary(id).padStart(64).substring(0, 42), 2) + epoch,
		);
	}
			...
			"2.0.3": [
				{
					title: "Added",
					type: "added",
					items: ["Added support for nitro boosting"],
				},
			],
			"2.0.7": [
				{
					title: "Fixed",
					type: "fixed",
					items: ["ContextMenu\'s should work again."],
				},
				{
					title: "Changed",
					type: "changed",
	...
	showWelcomeMessage() {
		let popup = this.alertText(
			"Welcome",
			`<p>GuildData is a plugin that displays the Guild Information such as the owner, when the server was created, when you joined, their verification level, etc. As well as user, channel, and role information which is very detailed.</p>
		...
		$(
			popup.querySelector("button.guilddata-welcome.guilddata-openchangelog"),
		).on("click", () => this.showChangelog());
	}
	...
	showChangelog(oldVersion, newVersion) {
		...
		for (const v in this.changelog) {
			...
			if (v == this.getVersion())
				c += ` style="background-color:#2d2d31;border:2px #a7a7a7;border-style:solid;border-radius:7px;padding:5px;"`;
