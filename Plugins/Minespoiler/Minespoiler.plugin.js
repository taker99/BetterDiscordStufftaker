...
class Minespoiler {
	...
	getDescription() {
		return "Send a game of minesweeper using spoilers. Write a message in the format: 'minesweeper:width height bombCount'. You can also write 'minesweeper:width height bombCount and here some text, %GAME% will put the field in the text.'";
	}
	getVersion() {
		return "1.0.10";
	}
		...
		new ZLibrary.Settings.SettingGroup(this.getName(), { shown: true })
			...
			.append(
				new ZLibrary.Settings.Switch(
					"Clyde mode",
					"If you enable this option games will be sent as a Clyde message. Only you will see them, nobody else.",
					this.settings.sendAsClyde,
					(e) => {
	...
	initialize() {
		...
		ZLibrary.PluginUpdater.checkForUpdate(
			this.getName(),
			this.getVersion(),
			"https://raw.githubusercontent.com/l0c4lh057/BetterDiscordStuff/master/Plugins/Minespoiler/Minespoiler.plugin.js",
		);
					...
					try {
						...
						for (let line of toSend.split("\n")) {
							if (current.length + line.length + 1 > 1850) {
								// actual character limit is 2000, but discord is retarded and sometimes some spoilers get cut off when using 2000 as limit
						...
						if (messages.length > 7) {
							ZLibrary.DiscordModules.MessageActions.sendBotMessage(
								cId,
								"Please use a smaller field size.",
							);
						} else {
							let send = () => {
		...
		if (this.settings.lastUsedVersion != this.getVersion()) {
			...
			this.saveSettings();
			BdApi.alert(
				"Minespoiler - Changelog",
				`Fixed revealing fields / flagging mines`,
			);
		}
	}
	...
	revealField(e) {
		...
		if (e.target.hasClass("spoilerText-3p6IlD")) {
			...
			if (!message.find(".markup-2BOw-j").innerHTML.includes("Minesweeper"))
				return;
			let isEmoji = false;
			for (let emoji of [
				":zero:",
			...
			let matches = message.innerHTML.match(
				/\((\d+)x(\d+) with (\d+) bombs(, -?\d+ remaining)?\)/,
			);
			if (!matches) return;
			let lost = false;
				...
				if (e.target.innerHTML.includes(":boom:")) {
					...
					let clickElement = function (element) {
						...
						if (
							...
							element.hasClass("flaggedAsMine")
						)
							return;
					...
					let checkSurroundingFields = function (element) {
						...
						if (position > width + 1 && position % width > 0)
							clickElement(siblings[position - width - 1]);
						...
						if (position % width < width - 1)
							clickElement(siblings[position + 1]);
						if (position % width > 0 && position < (height - 1) * width)
							clickElement(siblings[position + width - 1]);
						if (position < (height - 1) * width)
							clickElement(siblings[position + width]);
						if (position % width < width - 1 && position < (height - 1) * width)
							clickElement(siblings[position + width + 1]);
					};
			...
			if (revealedAllFields)
				for (let spoiler of message.findAll(".hidden-HHr2R9"))
					spoiler.classList.add("flaggedAsMine");
			if (lost)
				message.find(".markup-2BOw-j").find("em").innerHTML =
					`(${matches[1]}x${matches[2]} with ${matches[3]} bombs, you lost <img src="/assets/ef756c6ecfdc1cf509cb0175dd33c76d.svg" class="emoji" alt=":boom:" draggable="false">) <span class="minesweeper-retry">[Retry]</span>`;
			else if (revealedAllFields)
				message.find(".markup-2BOw-j").find("em").innerHTML =
					`(${matches[1]}x${matches[2]} with ${matches[3]} bombs, you won <img src="/assets/612f3fc9dedfd368820b55c4cf259c07.svg" class="emoji" alt=":tada:" draggable="false">) <span class="minesweeper-retry">[Retry]</span>`;
			else
				message.find(".markup-2BOw-j").find("em").innerHTML =
					`(${matches[1]}x${matches[2]} with ${matches[3]} bombs, ${parseInt(matches[3]) - message.querySelectorAll(".flaggedAsMine").length} remaining)`;
			if (message.find(".minesweeper-retry"))
				message.find(".minesweeper-retry").on("click", () => {
					let matches2 = message.innerHTML.match(
						/\((\d+x\d+ with \d+ bombs), (.*?)\)/,
					);
					message.find(".markup-2BOw-j").find("em").innerHTML =
						`(${matches2[1]})`;
					for (let spoiler of message.querySelectorAll(".spoilerText-3p6IlD")) {
						if (!spoiler.hasClass("hidden-HHr2R9"))
							$(spoiler).one("click", () => {
								spoiler.classList.remove("hidden-HHr2R9");
							});
	...
	flagMine(e) {
		if (!e.target.hasClass) return;
		if (
			e.target.hasClass("spoilerText-3p6IlD") &&
			e.target.hasClass("hidden-HHr2R9")
		) {
			let message = e.target.parentsUntil(".scrollerInner-2ircaP").reverse()[0];
			if (!message.find(".markup-2BOw-j").innerHTML.includes("Minesweeper"))
				return;
