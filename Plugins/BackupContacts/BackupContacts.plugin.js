...
class BackupContacts {
	...
	getDescription() {
		return "Create a backup of all your contacts and blocked users";
	}
	...
	initialize() {
		ZLibrary.PluginUpdater.checkForUpdate(
			this.getName(),
			"https://raw.githubusercontent.com/l0c4lh057/BetterDiscordStuff/master/Plugins/BackupContacts/BackupContacts.plugin.js",
		);
		this.onSwitch();
	}
	onSwitch() {
		if (
			document.getElementsByClassName("container-1r6BKw").length > 0 &&
			document.getElementsByClassName("backupContacts backupBtn").length == 0
		) {
			let friendsHeader = $(".container-1r6BKw")[0];
			let buttons = friendsHeader.find(".children-19S4PO .tabBar-ZmDY9v");
			let button1 = $(
				`<div class="backupContacts backupBtn item-3HknzM da-item item-PXvHYJ da-item" id="backupContacts-exportBtn" role="button" style="background-color: rgb(67, 181, 129); color: rgb(255, 255, 255);"><span aria-hidden="true">Export</span></div>`,
			)[0];
			let button2 = $(
				`<div class="backupContacts backupBtn item-3HknzM da-item item-PXvHYJ da-item" id="backupContacts-importBtn" role="button" style="background-color: rgb(67, 181, 129); color: rgb(255, 255, 255);"><span aria-hidden="true">Import</span></div>`,
			)[0];
	...
	exportContacts() {
		let relationships =
			ZLibrary.DiscordModules.RelationshipStore.getRelationships();
		let friends = [];
		let pending = [];
		let blocked = [];
	...
	async importContacts(data) {
		...
		for (let blocked of data["blocked"]) {
			...
			} else block.push(blocked);
		}
		for (let friend of data["friends"]) {
			if (relationshipUsers.includes(friend)) {
				if (relationships[friend] !== 1) sendRequest.push(friend);
			} else sendRequest.push(friend);
		}
		for (let pending of data["pending"]) {
			if (relationshipUsers.includes(pending)) {
				if (relationships[pending] !== 1 && relationships[pending] !== 4)
					sendPending.push(pending);
			} else sendPending.push(pending);
		}
		for (let uId of relationshipUsers) {
			if (
				(relationships[uId] === 1 || relationships[uId] === 4) &&
				!data["friends"].includes(uId) &&
				!data["pending"].includes(uId)
			)
				unfriend.push(uId);
			if (relationships[uId] === 2 && !data["blocked"].includes(uId))
				unblock.push(uId);
		}
		...
		let alert = this.alertText("Import Contacts", htmlString, () => {
			endRelationship = endRelationship.filter(
				(uId) => alert.find(`.backupContacts.endRelationship.id${uId}`).checked,
			);
			sendFriendRequest = sendFriendRequest.filter(
				(uId) => alert.find(`.backupContacts.sendRequest.id${uId}`).checked,
			);
			toBlock = toBlock.filter(
				(uId) => alert.find(`.backupContacts.block.id${uId}`).checked,
			);
	...
	async getUserById(uId) {
		...
		if (user) return user;
		let u = await ZLibrary.DiscordModules.APIModule.get(
			ZLibrary.DiscordModules.DiscordConstants.Endpoints.USER(uId),
		);
		if (u) return JSON.parse(u.text);
		return {
			username: "Could not resolve user",
			discriminator: "Could not resolve user",
		};
	}
	...
	getBlockedUsers() {
		...
		for (let uId in ZLibrary.DiscordModules.RelationshipStore.getRelationships()) {
			let status =
				ZLibrary.DiscordModules.RelationshipStore.getRelationships()[uId];
			if (status === 2) {
				blocked.push(uId);
			}
		}
	...
	addFriends(userIds) {
		let sendReq = () => {
			let uId = userIds[0];
			if (!uId) return;
			ZLibrary.DiscordModules.RelationshipManager.addRelationship(
				uId,
				0,
				4,
			).then((res) => {
	...
	saveFile(content, filename) {
		...
		dialog.showSaveDialog(
			{ defaultPath: filename, title: "Save Discord Contacts" },
			function (sel) {
				if (!sel) return;
				let fs = require("fs");
	...
	alertText(e, t, callback) {
		...
		let a =
			$(`<div class="modal-3c3bKg da-modal" style="opacity: 1; transform: scale(1) translateZ(0px); z-index: 9999999">
		...
		let modalRoot = document.querySelector(
			"#app-mount > div[data-no-focus-lock='true'] > div:not([class])",
		);
