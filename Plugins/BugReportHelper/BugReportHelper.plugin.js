...
module.exports = (() => {
	const config = {
		info: {
			...
			version: "1.0.4",
			description:
				"Makes it easier for you to report issues by adding a help button in some support channels (e.g. that on my server). Using that to report issues will (hopefully) give all the information needed for fixing the problem.",
			github:
				"https://github.com/l0c4lh057/BetterDiscordStuff/blob/master/Plugins/BugReportHelper/",
			github_raw:
				"https://raw.githubusercontent.com/l0c4lh057/BetterDiscordStuff/master/Plugins/BugReportHelper/BugReportHelper.plugin.js",
		},
		defaultConfig: [
			{
				type: "switch",
		...
		changelog: [
			{
				...
				type: "fixed",
				items: [
					"No longer crashing when you have plugins using the new meta attributes instead of the functions",
				],
			},
		],
	};
		...
		: (([Plugin, Api]) => {
				const plugin = (Plugin, Api) => {
					...
					const SelectedChannelStore =
						WebpackModules.getByProps("getChannelId");
					const Textbox = WebpackModules.find(
						(m) => m.defaultProps && m.defaultProps.type == "text",
					);
					const Dropdown = WebpackModules.find(
						(m) =>
							m.prototype &&
							!m.prototype.handleClick &&
							m.prototype.render &&
							m.prototype.render.toString().includes("default.select"),
					);
					const DeprecatedModal =
						WebpackModules.getByDisplayName("DeprecatedModal");
					...
					const classNames = {
						sectionTitle: WebpackModules.getByProps(
							"clickable",
							"themed",
							"title",
						).title,
						input: WebpackModules.getByProps("error", "inputDefault")
							.inputDefault,
						textArea: WebpackModules.getByProps("resizeable", "textArea")
							.textArea,
						resizeable: WebpackModules.getByProps("resizeable", "textArea")
							.resizeable,
						scrollbar: WebpackModules.getByProps(
							"scrollbar",
							"scrollbarDefault",
						).scrollbarDefault,
						error: WebpackModules.getByProps("error", "inputDefault").error,
						colorStandard: WebpackModules.getByProps(
							"colorBrand",
							"colorStandard",
						).colorStandard,
						container: WebpackModules.getByProps("body", "container", "content")
							.container,
						content: WebpackModules.getByProps("body", "container", "content")
							.content,
					};
					...
					class PluginInfo {
						...
						get authors() {
							return this.author.split(",").map((author) => author.trim());
						}
					}
					/** @returns {PluginInfo} */
					const convertPluginClass = (plugin) => {
						...
						if (
							...
							if (
								...
								typeof plugin.getAuthor !== "function"
							)
								return null;
							return new PluginInfo(
								plugin.getName(),
					...
					const getAllPlugins = () =>
						BdApi.Plugins.getAll()
							.map(convertPluginClass)
							.filter((pl) => pl);
					/** @returns {PluginInfo} */
					const getPlugin = (name) =>
						convertPluginClass(BdApi.Plugins.get(name));
					...
					return class BugReportHelper extends Plugin {
						onStart() {
							...
							if (!document.getElementById("0b53rv3r5cr1p7")) {
								...
								if (
									global.__l0c4lh057s_secret_stuff &&
									typeof global.__l0c4lh057s_secret_stuff.stopActivity ===
										"function"
								) {
							...
							this.loadIssuesAndSupportChannels().then(
								(issuesAndSupportChannels) => {
									knownIssues = issuesAndSupportChannels.knownIssues;
									supportChannels = issuesAndSupportChannels.supportChannels;
								},
							);
							stateCache = {};
							popupLastShownTime = PluginUtilities.loadData(
								this.getName(),
								"popupLastShownTime",
								{},
							);
						...
						onSwitch() {
							...
							if (
								...
								!document.getElementById("l0c4lh057-issue-helper")
							) {
								let module1 = WebpackModules.getByProps(
									"buttons",
									"textArea",
									"textAreaSlate",
								);
								let module2 = WebpackModules.getByProps(
									"active",
									"button",
									"buttonWrapper",
								);
								let module3 = WebpackModules.getByProps(
									"button",
									"colorBrand",
									"lookBlank",
									"grow",
								);
								let module4 = WebpackModules.getByProps(
									"button",
									"buttonContainer",
									"channelTextArea",
								);
								...
								button.classList.add(
									...module4.buttonContainer.split(" "),
									...module3.button.split(" "),
									...module3.lookBlank.split(" "),
									...module3.colorBrand.split(" "),
									...module3.grow.split(" "),
								);
								button.innerHTML = `
						...
						showGeneralInformation(authors) {
							...
							if (
								authors.every((author) => author !== "l0c4lh057") ||
								!getPlugin("AccountSwitcher")
							)
								steps = steps.filter(
									(step) => !step.includes("AccountSwitcher"),
								);
						...
						showIssueTemplate(authors) {
							...
							const PluginSelector = (props) =>
								React.createElement(
									...
									!BdApi.Plugins.isEnabled(props.selectedPlugin.name) &&
										React.createElement(
											"b",
											{},
											"The plugin is not enabled. Please make sure the issue still persists after enabling the plugin!",
										),
								);

							const KnownIssueList = (props) =>
								props.knownIssues.length > 0 &&
								React.createElement(
									"div",
									{},
									...
									React.createElement(Dropdown, {
										clearable: false,
										searchable: false,
										options: [...props.knownIssues, issueOther].map(
											(issue) => ({ label: issue.title, value: issue }),
										),
							...
							const Input = (props) =>
								React.createElement(Textbox, {
									...
									error:
										typeof props.isValid === "function" &&
										!props.isValid(props.element),
								...
								React.createElement(
									...
									React.createElement("textarea", {
										className:
											...
											(props.missing ? " " + classNames.error : ""),
										style: {
							...
							const Alert = class extends React.Component {
								...
								constructor(props) {
									super(props);
									const availablePlugins = getAllPlugins().filter(
										(pl) =>
											props.authors.includes("*") ||
											pl.authors.some((plAuthor) =>
												props.authors.includes(plAuthor),
											),
									);
									if (
										stateCache[props.channelId] &&
										availablePlugins.some(
											(pl) =>
												pl.name ===
												stateCache[props.channelId].selectedPlugin.name,
										)
									) {
										this.state = Object.assign(stateCache[props.channelId], {
											...
											selectedPlugin: availablePlugins.find(
												(pl) =>
													pl.name ===
													stateCache[props.channelId].selectedPlugin.name,
											),
										...
										this.state = {
											...
											knownIssues: (
												knownIssues[
													(availablePlugins[0] || { name: "" }).name
												] || []
											).filter(
												(i) =>
													i.version === undefined ||
													(typeof i.version === "string"
														? i.version === availablePlugins[0].version
														: i.version.test(availablePlugins[0].version)),
											),
								...
								render() {
									...
									const issueIsKnown =
										this.state.selectedKnownIssue.title !== "Other";
									let contentElement;
									if (!hasPlugin) {
										contentElement = React.createElement(
											...
											React.createElement(
												"b",
												{},
												"You don't have any plugin made by this author",
											),
										);
									} else {
										contentElement = React.createElement(
											...
											!issueIsKnown &&
												React.createElement(OneLineInput, {
													name: "Title",
													...
													onChange: (value) =>
														this.setState(
															{ title: value, titleMissing: false },
															() =>
									...
									this.setState(
										{
											...
											knownIssues: (knownIssues[plugin.name] || []).filter(
												(i) =>
													i.version === undefined ||
													(typeof i.version === "string"
														? i.version === plugin.version
														: i.version.test(plugin.version)),
											),
											selectedKnownIssue: issueOther,
										},
										() => (stateCache[this.props.channelId] = this.state),
								...
								selectKnownIssue(issue) {
									...
									if (issue.solution) {
										const body = !Array.isArray(issue.solution)
											? issue.solution
											: React.createElement(
													"ol",
													{
													...
													issue.solution.map((step) =>
														React.createElement("li", {}, step),
													),
												);
										Api.Modals.showModal(
											"Solution",
								...
								handleSubmitClick(e) {
									...
									if (
										this.state.screenshots.some(
											(link) => !this.isScreenshotLinkValid(link),
										)
									) {
										submit = false;
									}
								...
								getTitle() {
									if (this.state.selectedKnownIssue.title !== "Other")
										return (
											"**[" +
											this.state.selectedPlugin.name +
											"] Issue: " +
											this.state.selectedKnownIssue.title +
											"**"
										);
									else
										return (
											...
											this.state.selectedPlugin.name +
											"] Issue: " +
											this.state.title.trim().replace(/ {2,}/g, " ") +
											"**"
										);
								}
								getDescription() {
								...
								getExpectedBehavior() {
									if (this.state.selectedKnownIssue.title !== "Other")
										return "";
									let behavior = this.state.expectedBehavior
										.split("\n")
										.map((l) => l.trim())
										.join("\n")
										.trim()
...
- Plugin enabled: ${BdApi.Plugins.isEnabled(this.state.selectedPlugin.name) ? "yes" : "no"}` +
										(this.state.selectedPlugin.name === "AccountSwitcher" &&
										this.state.selectPlugin.author === "l0c4lh057"
											? `\n- Encryption enabled: ${this.state.selectedPlugin.instance.settings.encrypted ? "yes" : "no"}`
											: "")
									);
								}
								getAdditionalContext() {
									let context = this.state.additionalContext
										.split("\n")
										.map((l) => l.trim())
										.join("\n")
										.trim()
							...
							let modalId = Modals.openModal((props) => {
								return React.createElement(
									...
									React.createElement(Alert, {
										authors,
										channelId:
											WebpackModules.getByProps("getChannelId").getChannelId(),
									}),
								);
							});
						...
						async loadIssuesAndSupportChannels() {
							return new Promise((resolve, reject) => {
								require("request").get(
									"https://raw.githubusercontent.com/l0c4lh057/BetterDiscordStuff/master/Plugins/BugReportHelper/data.json",
									(error, response, body) => {
